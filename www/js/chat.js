...
$("#chat_room_modal").addClass('active');
 ...
 $.post("http://photomeeting.net/chat_list_app.php",
   {
    no:no,
    memberuid:memberuid
...
function save_chat() {
  console.log(memberuid);
	...
	if (!chat_msg){
		...
		$.post("http://photomeeting.net/chat_save_app.php",
   {
   	room_no:room_no,
   	memberuid:memberuid,
   	chat_msg:chat_msg
...
function make_chat_open_job() {
   var modal = UIkit.modal("#make_room_job_modal", {center: true});
    ...
    modal.show();
      ...
      $.post("http://photomeeting.net/job_subject_list_app.php",
   {
...
function make_room_job() {
  ...
  var subject=$("#list_subject").val();
   ...
   $.post("http://photomeeting.net/make_room.php",
   {
    uid:memberuid,
    subject:subject,
    room_kind:2
...
make_chat_close();
  ...
  function delete_room(no) {
    var uid=no;
     $.post("http://photomeeting.net/delete_room_app.php",
   {
    uid:uid
...
// 대화삭제 
 ...
 function delete_chat_list(no,room_no) {
  ...
  $.post("http://photomeeting.net/delete_chat_list_app.php",
   {
...
// 대화갱신
 ...
 function check_new_chat() {
  ...
  if (check_chat=="t") {
   ...
   $.post("http://photomeeting.net/check_new_chat_no_app.php",
   {
...
function reload_chat(room_no,last_no) {
  ...
  console.log('last_no'+last_no);
   $.post("http://photomeeting.net/check_new_chat_app.php",
   {
